...
class TestProcedures:
    ...
    def test_args(self):
        ...
        assert len(refs) == 2
        assert mod[refs[0]].name == "y"
        assert mod[refs[1]].name == "x"
    ...
    def test_intent(self):
        ...
        assert mod[refs[0]].properties.attributes.intent == "IN"
        assert mod[refs[1]].properties.attributes.intent == "OUT"

    def test_value(self):
        refs = self.mod["func_int_value"].properties.formal_argument
        assert mod[refs[0]].properties.attributes.value

    def test_optional(self):
        refs = self.mod["sub_int_opt"].properties.formal_argument
        assert mod[refs[0]].properties.attributes.optional
